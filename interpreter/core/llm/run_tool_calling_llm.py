...
edit_tool_schema = {
    ...
    "function": {
        ...
        "description": (
            ...
            "  sed   — sed script for one-line regex search/replace (s///). A multi-line script (several s/// lines) is OK, but do NOT use a/i/c for multi-line insert blocks — use gawk or patch. BRE: bare + is literal, \\+ is a quantifier. If text contains /, use another delimiter (s|old|new|).\n"
            "  gawk  — GNU awk filter: program must print the new file on stdout per line (e.g. { print }). END-only output replaces the whole file, so include per-line print when transforming.\n"
