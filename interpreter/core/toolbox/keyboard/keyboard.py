...
class Keyboard:
    ...
    def write(self, text, interval=None, delay=0.30, **kwargs):
        ...
        if interval:
            ...
            try:
                clipboard_history = self.toolbox.clipboard.view()
            except:
                pass
            ...
            if len(lines) < 5:
                for i, line in enumerate(lines):
                    ...
                    self.toolbox.clipboard.copy(line)
                    self.toolbox.clipboard.paste()
            else:
                # just do it all at once
                self.toolbox.clipboard.copy(text)
                self.toolbox.clipboard.paste()
            ...
            try:
                self.toolbox.clipboard.copy(clipboard_history)
            except:
                pass
