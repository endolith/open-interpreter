...
class Skills:
    ...
    def import_skills(self):
        ...
        previous_save_skills_setting = self.toolbox.save_skills

        self.toolbox.save_skills = False
...
2. After I explicitly tell you the name of the skill (I may tell you to proceed which is not the name— if I do say that, you probably need more information from me, so tell me that), after you get the proper name, execute `toolbox.skills.new_skill.name = "{INSERT THE SKILL NAME FROM QUESTION #1}"`.
...
3. Ask me if you completed the step correctly.
    a. (!!!!!!!!!!!! >>>>>> THIS IS CRITICAL. DO NOT FORGET THIS.) IF you completed it correctly, run `toolbox.skills.new_skill.add_step(step, code)` where step is a generalized, natural language description of the step, and code is the code you ran to complete it.
    b. IF you did not complete it correctly, try to fix your code and ask me again.
4. If I say the skill is complete, or that that was the last step, run `toolbox.skills.new_skill.save()`.
...
def {normalized_name}(step=0):
    ...
    if step < len(steps):
        if isinstance(steps[step], str):
            ...
            toolbox.mouse.click(steps[step]["element"], icon_dimensions=steps[step]["icon_dimensions"]) # Instructed click
        if step + 1 < len(steps):
