...
class Display:
    ...
    def screenshot(
        ...
        # We should probably remove self.toolbox.emit_images for this reason.

        # if not self.toolbox.emit_images and force_image == False:
        #     screenshot = self.screenshot(show=False, force_image=True)

        #     description = self.toolbox.vision.query(pil_image=screenshot)
        #     print("A DESCRIPTION OF WHAT'S ON THE SCREEN: " + description)

        #     if self.toolbox.max_output > 600:
        ...
        #         print(
        #             format_to_recipient(
        #                 "To receive the text above as a Python object, run toolbox.display.get_text_as_list_of_lists()",
        #                 "assistant",
        #             )
        #         )
            ...
            if active_app_only:
                ...
                if active_window:
                    ...
                    message = format_to_recipient(
                        "Taking a screenshot of the active app. To take a screenshot of the entire screen (uncommon), use toolbox.view(active_app_only=False).",
                        "assistant",
                    )
                    print(message)
                else:
                ...
                message = format_to_recipient(
                    "Taking a screenshot of the entire screen.\n\nTo focus on the active app, use toolbox.display.view(active_app_only=True).",
                    "assistant",
                )
                print(message)
