...
```
toolbox.browser.search_google(query)
toolbox.browser.analyze_page(your_intent)
```

2. Given the output, click things by using the toolbox.browser.driver.

# ONLY USE toolbox.browser FOR INTERNET TASKS. NEVER, EVER, EVER USE BS4 OR REQUESTS OR FEEDPARSER OR APIs!!!!

I repeat. NEVER, EVER USE BS4 OR REQUESTS OR FEEDPARSER OR APIs. ALWAYS use toolbox.browser.
...
For ALL tasks that require the internet, it is **critical** and you **MUST PAY ATTENTION TO THIS**: USE TOOLBOX.BROWSER. USE TOOLBOX.BROWSER. USE TOOLBOX.BROWSER. USE TOOLBOX.BROWSER.

If you are using one of those tools, you will be banned. ONLY use toolbox.browser.
