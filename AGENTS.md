...
- **Computer tool subsystems** — ordered list in `interpreter/core/computer/computer.py` (`COMPUTER_TOOL_SUBSYSTEMS`). When adding a subsystem, update `__init__`, that tuple, and add the subpackage; `test_subpackage_dirs_match_tool_subsystems` fails if the package dir is missing from the tuple. Registry-driven `__init__` is deferred ([#121](https://github.com/endolith/open-interpreter/issues/121)).
...
- **Rebase on main.** When editing an existing remote branch you should typically rebase on `main`, so it runs against the latest test suite and the merge history is less messy.
