...
- **MUST REDACT:** Fake secrets in two groups:
  ...
  - Format-specific lines that match built-in detectors: AWS, GitHub classic personal access tokens, Slack, Basic auth, Stripe, SendGrid, Twilio, Discord, JWT (and KeywordDetector in code-style snippets).
  - Fine-grained GitHub personal access tokens that match the custom token-format pattern, covered by `tests/core/test_sanitize_secrets.py`.
