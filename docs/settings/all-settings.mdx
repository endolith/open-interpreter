...
- `reasoning_effort`: `"low"`, `"medium"`, or `"high"` (only when the model supports it). Some models only accept a specific subset (e.g. GLM-5.3 accepts `low`/`high`/`max`); an unsupported value is dropped with a note rather than sent.
- `include_reasoning`: `true` (default for supported models) or `false` to disable reasoning tokens entirely. Setting it to `false` also disables `reasoning_effort`.
- Some endpoints **mandate reasoning** (e.g. `z-ai/glm-5.3-flash`, reported `reasoning.mandatory` in OpenRouter's model list). Reasoning can't be turned off there — Open Interpreter detects this and ignores `include_reasoning: false` (with a one-time note) instead of sending a request the API rejects with `400 "Reasoning is mandatory"`. Use `reasoning_effort: "low"` as the floor for such models.
