...
<Card
  ...
  href="#toolbox"
>
  Modify the `interpreter.toolbox`, which provides convenience functions for the AI agent.
</Card>
...
Controls how much a reasoning model thinks before answering. Reasoning/thinking models (e.g. `deepseek/`, `z-ai/glm-5.3`, some OpenRouter models) emit separate reasoning tokens before their visible reply; these tokens consume time and `max_tokens` budget, and a model that "thinks endlessly in circles" can be reined in by lowering the effort or disabling reasoning entirely.

- `reasoning_effort`: how hard a reasoning model should think. Accepted values are model-specific (e.g. `deepseek/` and `z-ai/glm-5.3` accept `low`/`high`/`max`) and change over time, so the value you set is sent as-is rather than pre-filtered by Open Interpreter. If the provider rejects it, the provider's own error is shown; when that error names `reasoning_effort`, a short note follows repeating the value that was actually sent.
- `include_reasoning`: `true` (default for supported models) or `false` to disable reasoning tokens entirely. It is also sent as-is. If an endpoint mandates reasoning, it returns its own error (`400 "Reasoning is mandatory…"`), which is surfaced rather than pre-empted.
- Setting `include_reasoning: false` together with a `reasoning_effort` is contradictory: a model told not to think ignores the effort (providers return success with no reasoning). Both are still sent as configured, and a one-time note explains that the effort will have no effect.

<CodeGroup>

```python Python
interpreter.llm.reasoning_effort = "low"  # or "medium", "high"
interpreter.llm.include_reasoning = False  # disable thinking tokens
```

```yaml Profile
llm:
  reasoning_effort: "low"
  # include_reasoning: true  # set false to disable thinking entirely
```
...
### Sanitize secrets (API keys, passwords)

When using a **remote API model**, Open Interpreter runs all message text (including code outputs) through [bc-detect-secrets](https://github.com/bridgecrewio/detect-secrets) to redact API keys, passwords, and other secrets before sending to the API—for example when the model runs `print(os.environ)` or code that echoes credentials. **Local models** (e.g. `ollama/`, `local/`, `jan/`) are not sanitized by default, since the data stays on your machine.

- **`auto`** (default): Sanitize for remote (API) models; skip for local models.
- **`on`**: Always sanitize (including local).
- **`off`**: Never sanitize.

This is separate from **Safe Mode** (`safe_mode`), which uses Semgrep to scan generated code for vulnerabilities before you run it. Safe Mode is optional (`pip install open-interpreter[safe]` adds semgrep).

<CodeGroup>

```bash Terminal
interpreter --sanitize_secrets auto
interpreter --sanitize_secrets on
interpreter --sanitize_secrets off
```

```python Python
interpreter.llm.sanitize_secrets = "auto"  # or "on" or "off"
```

```yaml Profile
llm:
  sanitize_secrets: auto
```
...
The `toolbox` object in `interpreter.toolbox` is a collection of Python convenience functions that the AI agent can use to interact with the operating system and accomplish tasks (e.g., display, mouse, keyboard, browser, files, calendar, contacts, mail, SMS, vision, skills, docs, AI utilities).

### Offline

Running the `toolbox` in offline mode will disable some online features, like the hosted [Toolbox API](https://api.openinterpreter.com/). Inherits from `interpreter.offline`.

<CodeGroup>

```python Python
interpreter.toolbox.offline = True
```

```yaml Profile
toolbox.offline: True
```
...
This is primarily used for debugging `interpreter.toolbox`. Inherits from `interpreter.verbose`.
...
The `emit_images` attribute in `interpreter.toolbox` controls whether the toolbox should emit images or not. This is inherited from `interpreter.llm.supports_vision`.

This is used for multimodel vs. text only models. Running `toolbox.display.view()` will return an actual screenshot for multimodal models if `emit_images` is True. If it's False, `toolbox.display.view()` will return all the text on the screen.

Many other functions of the toolbox can produce image/text outputs, and this parameter controls that.
...
Include the toolbox API in the system message. The default is False and won't import the toolbox API automatically
